...
/**
 *
 * @param {string} tlv tlv string
 * @param {string} tagToUpdate tag which needs to be udated
 * @param {string} valueToUpdate value for update
 * @param {object} config: { shouldUpdateAll: boolean, rootToUpdate: number } - shouldUpdateAll: true to update at all occurences, rootToUpdate: 1 to update a tag under a root of position 1
 * @returns
 */
TLV.update = function (tlv, tagToUpdate, valueToUpdate, config) {
	const encoding = config?.encoding;
	const shouldUpdateAll = config?.shouldUpdateAll;
	const rootToUpdate = config?.rootToUpdate;
	const roots = TLV.parseMultipleRoots(tlv);
	let result = "";
	let isFound = false;
	for (let itr = 0; itr < roots.length; itr++) {
		const current = roots[itr];
		if ((isFound && !shouldUpdateAll) || (rootToUpdate !== undefined && itr !== rootToUpdate)) {
			result += current;
			continue;
		}
		const constructed = findAndReplace(current);
		if (current !== constructed || itr === rootToUpdate) isFound = true;
		result += constructed.toUpperCase();
	}
	return result;
 ...
 *
 * @param {string} tlv tlv string
 * @param {string} tagUnder tag where tagToAdd and valueToAdd need to be added
 * @param {{tagToAdd, valueToAdd}} param2 tagToAdd: tag which needs to be added, valueToAdd: value which needs to be added
 * @param {object} config: { shouldAddAll: boolean, rootToAdd: number } - shouldAddAll: true to add at all occurences, rootToAdd: 1 to add a tag under a root of position 1
 * @returns
 */
TLV.add = function (tlv, tagUnder, { tagToAdd, valueToAdd }, config) {
	const encoding = config?.encoding;
	const shouldAddAll = config?.shouldAddAll;
	const rootToAdd = config?.rootToAdd;
	const roots = TLV.parseMultipleRoots(tlv);
	...
	for (let itr = 0; itr < roots.length; itr++) {
		const current = roots[itr];
		if ((isFound && !shouldAddAll) || (rootToAdd !== undefined && itr !== rootToAdd)) {
			result += current;
			continue;
		}
		const constructed = findAndAdd(current);
		if (current !== constructed || itr === rootToAdd) isFound = true;
		result += constructed.toUpperCase();
	}
 ...
 *
 * @param {string} tlv tlv string
 * @param {string} tagToDelete tag which needs to be deleted
 * @param {object} config: { shouldDeleteAll: boolean, rootToDelete: number } - shouldDeleteAll: true to delete all occurences, rootToDelete: 1 to delete a tag under a root of position 1
 * @returns
 */
TLV.delete = function (tlv, tagToDelete, config) {
	// const constructedTLV = TLV.parse(tlv, encoding);
	// const constructed = findAndDelete(constructedTLV);
	// return constructed.toUpperCase();

	const encoding = config?.encoding;
	const shouldDeleteAll = config?.shouldDeleteAll;
	const rootToDelete = config?.rootToDelete;
	const roots = TLV.parseMultipleRoots(tlv);
	let result = "";
	let isFound = false;
	for (let itr = 0; itr < roots.length; itr++) {
		const current = roots[itr];
		if ((isFound && !shouldDeleteAll) || (rootToDelete !== undefined && itr !== rootToDelete)) {
			result += current;
			continue;
		}
		const constructed = findAndDelete(current);
		if (current !== constructed || itr === rootToDelete) isFound = true;
		result += constructed.toUpperCase();
	}
	...
	/**
	 * Constructs the TLV (Tag-Length-Value) value for a given tag and deletes its value if found.
	 * @param {TLV} tlv
	 * @returns
	 */
 ...
 * @returns {TLV}
 */
TLV.parseMultipleRoots = function (data, encoding) {
	let itr = 0;
	let completed;
	const result = [];
	while (itr < data.length) {
		const parsed = TLV.parse(data.slice(itr));
		completed = parsed.tag.length + parsed.bLength.length + parsed.value.length + 1;
		itr += completed;
		result.push(parsed);
	}
	return result;
