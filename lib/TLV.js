...
TLV.update = function (tlv, tagToUpdate, valueToUpdate, config) {
	...
	for (let itr = 0; itr < roots.length; itr++) {
		...
		if (isFound && !shouldUpdateAll) {
			result += current;
			continue;
		}
		const constructed = findAndReplace(current);
		if (current !== constructed) isFound = true;
...
function getNumberOfRoots(tlv) {
	let itr = 0;
	let completed;
	let result = [];
	while (itr < tlv.length) {
		const parsed = TLV.parse(tlv.slice(itr));
		completed = parsed.tag.length + (parsed.value.length / 2).length.toString(16) + parsed.value.length;
		itr += completed;
		result.push(parsed);
	}
	return result;
